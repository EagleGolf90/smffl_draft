...
let allPlayers = []; // Will be populated after players are loaded
...
function initializeDraft() {
  ...
  // Populate allPlayers from the loaded availablePlayers
  allPlayers.length = 0; // Clear array
...
function populatePositionDropdown(selectedPosition = "") {
  ...
  positions.forEach((position) => {
    ...
    const availableCount = draftState.availablePlayers.filter(
...
function populatePlayerDropdown(selectedPosition = null) {
  ...
  const availablePositionPlayers = draftState.availablePlayers.filter(
    (p) => p.position === selectedPosition,
  );

  // Sort by name
  const sortedPlayers = [...availablePositionPlayers].sort((a, b) =>
...
function loadDraftState() {
  ...
  if (saved) {
    ...
    if (
      draftState.timeRemaining === undefined ||
...
function syncToLocalStorage() {
  ...
  const teamName =
    (window.teamNames && window.teamNames[currentTeam - 1]) ||
    `Team ${currentTeam}`;
  ...
  localStorage.setItem(
    "draftTimeRemaining",
    draftState.timeRemaining.toString(),
  );
}
...
function updateDraftInfo() {
  ...
  const activeColumn = document.querySelector(
    `.team-column[data-team="${activeTeam}"]`,
  );
  if (activeColumn) {
...
function updateTeamColumns() {
  ...
  function createTeamHeader(teamIndex) {
    const teamName =
      (window.teamNames && window.teamNames[teamIndex - 1]) ||
      `Team ${teamIndex}`;
    const teamOwner =
      (window.teamOwners && window.teamOwners[teamIndex - 1]) || "";
...
function skipPick() {
  const confirmSkip = confirm(
    "Skip the current pick? This will record a skipped pick for the team on the clock.",
  );
...
function resetDraft() {
  const confirmReset = confirm(
    "Are you sure you want to reset the entire draft? This will clear all picks and cannot be undone.",
  );
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!importedData.draftState || !importedData.version) {
          alert(
            "Invalid draft file format. Please select a valid SMFFL draft file.",
          );
          return;
        }
        ...
        alert(
          "Error importing draft file. Please make sure the file is valid JSON.",
        );
      }
    };
...
function updateTimerDisplay() {
  document.getElementById("timerDisplay").textContent = formatTime(
    Math.max(0, draftState.timeRemaining),
  );
}
...
function applySettings() {
  ...
  const confirmChange = confirm(
    "Changing settings will reset the draft. Continue?",
  );
...
document
  .getElementById("exportDraft")
  .addEventListener("click", exportDraftToJSON);

document
  .getElementById("importDraft")
  .addEventListener("click", importDraftFromJSON);
...
// Wait for both team names and players to be loaded before initializing
let teamNamesReady = false;
let playersReady = false;

function checkAndInitialize() {
  if (teamNamesReady && playersReady) {
    console.log("Both team names and players loaded, initializing draft...");
    initializeDraft();
  }
}

window.addEventListener("teamNamesLoaded", () => {
  console.log("Team names loaded");
  teamNamesReady = true;
  checkAndInitialize();
});
...
function updatePositionsTracker() {
  ...
  for (let i = 1; i <= draftState.numTeams; i++) {
    ...
    const teamName =
      (window.teamNames && window.teamNames[i - 1]) || `Team ${i}`;
